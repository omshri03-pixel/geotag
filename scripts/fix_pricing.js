...
const pricingRegex = /(<section className="py-24 px-4 sm:px-6 lg:px-8 w-full relative z-10 bg-\[\#F25623\] text-\[\#171717\]">\n<div className="max-w-7xl mx-auto w-full">\n\n        \n        <div className="text-center max-w-3xl mx-auto space-y-4 mb-12 sm:mb-16">\n          <span className="text-\[\#171717\] font-mono text-xs uppercase tracking-widest font-black bg-\[\#171717\]\/20 border border-\[\#F25623\]\/30 px-3.5 py-1.5 rounded-full">)([\s\S]*?)(<\/section>)/;

// Wait, I should just use index based replacement because regexes are failing due to formatting
const priceStartStr = '{/* ==================== SCALE MAP LISTINGS: PRICING (SECTION 6) ==================== */}';
const priceStartIndex = content.indexOf(priceStartStr);

if (priceStartIndex !== -1) {
    const nextSectionStr = '{/* ==================== TESTIMONIALS SECTION';
    const nextSectionIndex = content.indexOf(nextSectionStr);
    
    const newPricing = `      {/* ==================== SCALE MAP LISTINGS: PRICING (SECTION 6) ==================== */}
      <section id="pricing" className="py-24 px-4 sm:px-6 lg:px-8 w-full relative z-10 bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-4 max-w-5xl mx-auto w-full">
          ...
          <div className="bg-[#171717] rounded-[28px] p-6 sm:p-8 border-2 border-[#F25623] flex flex-col justify-between space-y-8 relative shadow-2xl text-left w-full scale-105 z-10">
...
`;
    ...
    content = content.substring(0, priceStartIndex) + newPricing + content.substring(nextSectionIndex);
}

fs.writeFileSync('./src/app/page.tsx', content, 'utf8');
console.log("Pricing successfully hard-coded to perfect white contrast layout.");
