...
if (defaultUrl) {
  variations.push({
    name: 'DATABASE_URL from backend/.env',
    url: defaultUrl
  });
}
...
async function runTests() {
  for (const item of variations) {
    ...
    const pool = new Pool({
      ...
      ssl: item.url.includes('localhost') || item.url.includes('127.0.0.1') ? false : { rejectUnauthorized: false }
    });
  ...
  console.log("\n❌ Connection failed with all variations.");
}
